...
<xsl:stylesheet 
	...
	<xsl:template match="/soapenv:Envelope/soapenv:Body/pat:*">
		<xsl:variable name="eventName" select="concat(translate(substring(local-name(.), 1, 1), $lowercase, $uppercase), substring(local-name(.), 2))"/>
		<soapenv:Envelope>
			<soapenv:Header/>
			<soapenv:Body>
	...
	<xsl:template match="/soapenv:Envelope/soapenv:Body/pat:*/orderType">
		<wfm:Order_Type><xsl:apply-templates/></wfm:Order_Type>
	</xsl:template>
